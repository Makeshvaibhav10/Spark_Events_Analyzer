...
import unittest
import json
from spark_event_analyzer.metrics_engine_enhanced import MetricsEngine

class TestMetricsEngineEnhanced(unittest.TestCase):

    def test_compute_comprehensive_metrics(self):
        with open('/Users/makeshvaibhav/Desktop/spark-event-analyzer/tests/sample_logs/sample_log_enhanced.json', 'r') as f:
            events = json.load(f)
        
        engine = MetricsEngine()
        metrics = engine.compute_comprehensive_metrics(events)
        ...
        self.assertIn('gc_pressure_level', metrics['memory_metrics'])
        ...
        self.assertIn('shuffle_time_percentage', metrics['shuffle_metrics'])
